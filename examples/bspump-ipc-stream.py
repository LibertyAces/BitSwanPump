...
class StreamSinkPipeline(bspump.Pipeline):
	...
	def __init__(self, app, pipeline_id):
		...
		self.build(
			...
			# bspump.ipc.StreamSink(app, self, config={"socket_path": "/tmp/bspump-ipc-stream.sock"}),
			bspump.ipc.StreamSink(app, self, config={"port": 8083}),
		)
...
class StreamSourcePipeline(bspump.Pipeline):
	...
	def __init__(self, app, pipeline_id):
		super().__init__(app, pipeline_id)
		self.build(
			# bspump.ipc.StreamSource(app, self, config={"socket_path": "/tmp/bspump-ipc-stream.sock"}),
			bspump.ipc.StreamSource(app, self, config={"port": 8083}),
