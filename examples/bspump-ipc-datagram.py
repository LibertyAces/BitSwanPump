...
class StreamSinkPipeline(bspump.Pipeline):
	...
	def __init__(self, app, pipeline_id):
		...
		self.build(
			...
			# bspump.ipc.DatagramSink(app, self, config={"socket_path": "/tmp/bspump-ipc-datagram.sock"}),
			bspump.ipc.DatagramSink(app, self, config={"port": 8083}),
		)
...
class StreamSourcePipeline(bspump.Pipeline):
	...
	def __init__(self, app, pipeline_id):
		super().__init__(app, pipeline_id)
		self.build(
			# bspump.ipc.DatagramSource(app, self, config={"socket_path": "/tmp/bspump-ipc-datagram.sock"}),
			bspump.ipc.DatagramSource(app, self, config={"port": 8083}),
